...
def idle_draw(points):
    ...
    glColor3f(1.0, 1.0, 1.0)                           # set mode to 2d
    draw_grids()
    draw_line()
...
def rotate_vertices(current_vertices, angle, pointA, pointB):
    ...
    tempy = np.append(tempy,current_vertices[:,1])
    angle *= DEGREE_TO_RAD
    current_vertices[:,0] = ((tempx-pointA)*math.cos(angle))-((tempy-pointB)*math.sin(angle))
    current_vertices[:,1] = ((tempx-pointA)*math.sin(angle))+((tempy-pointB)*math.cos(angle))
    return current_vertices
